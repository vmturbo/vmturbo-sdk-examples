...
import com.vmturbo.platform.sdk.common.DTO.ModelEnum;
...
/**
 * A sample implementation of the file probe. This probe reads data from an XML topology file 
 * and generates entity DTOs for the target.
 */
public class FileProbe extends AbstractProbe {
	 ...
	 */
	private static final Set<Commodity> commWithKeySet = Sets.newHashSet(
			Commodity.StorageClusterCommodity,
			Commodity.DatastoreCommodity,
			Commodity.DSPMAccessCommodity,
			Commodity.Extent,
			Commodity.ClusterCommodity,
			Commodity.NetworkCommodity,
	 ...
	 */
	private Map<String, String> commToSeMap = new HashMap<String, String>();

	/**
	 * Map of commodity bought map indexed by entity DTO.
	 * The commodity bought map is a map of commodity DTO indexed by the consumes uuid
	 */
	private Map<EntityDTO, Map<String, CommodityDTO>> edToConsCommBoughtMap = new HashMap<EntityDTO, Map<String, CommodityDTO>>();
	...
	private EntityDTO generateEntityDTO(Element elem) {
		...
		if (name.equals("")) name = elem.getAttribute(ATTR_NAME);
	...
	private CommodityDTO generateCommDTO(Element elem) {
		...
		try {
			...
			if (capacity != "") cd.setCapacity(Float.parseFloat(capacity));
		} catch (Exception e){
		...
		try {
			...
			if (used != "") cd.setUsed(Float.parseFloat(used));
		} catch (Exception e){
	...
	protected Set<EntityDTO> discoverTarget(Map<String, String> accountValues) {
		...
		try {
			...
			for (int i = 0; i < mktNodeListLen; i++) {
				...
				for (int j = 0; j < entityNodeListLen; j++) {
					...
					if (ed.getEntity().equals(Entity.ActionManager)) continue; // Skip the ActionManager SE's
					if (ed.getEntity().equals(Entity.Application)) continue; // Skip the Application SE's
		...
		for (EntityDTO ed : entityDTOSet) {
			...
			ed.setCommodBoughtMap(commBoughtMap);
		}
	 ...
	 */
	protected TemplateDTO generateTemplateDTO(Entity entityType, List<CommodityDTO> commSoldList, CommBoughtPair... commBoughtPair) {
		Map<Provider, List<CommodityDTO>> commBoughtMap = new HashMap<Provider, List<CommodityDTO>>();
		TemplateDTO tempDTO = new TemplateDTO(entityType, commSoldList, commBoughtMap, TemplateType.Base, 0);

		int commBoughtPairLen = commBoughtPair.length;
		for (int i = 0; i < commBoughtPairLen; i++) {
			tempDTO.commBoughtMap.put(commBoughtPair[i].provider, commBoughtPair[i].commList);
		}
		return tempDTO;
	}

	/**
	 * Generate a commodity DTO with used = 1f, and capacity = 100f and 
	 * key = "foo" if it is in commWithKeySet; otherwise, key is set to null.
	 ...
	 * 		Virtual machines sell commodities to applications.
	 * 
	...
	protected Set<TemplateDTO> getSupplyChainDefinition() {
		...
		// Data center DTO
		List<CommodityDTO> dcCommSold = getCommList(Commodity.Power, Commodity.Space, Commodity.Cooling);
		...
		// DiskArray DTO
		List<CommodityDTO> daCommSold = getCommList(Commodity.Extent, Commodity.StorageAccess, Commodity.StorageLatency);
		templateDTOSet.add( generateTemplateDTO(Entity.DiskArray, daCommSold) );

		// Storage DTO
		List<CommodityDTO> stCommSold = getCommList(Commodity.StorageAmount, Commodity.StorageProvisioned, Commodity.StorageAccess, Commodity.StorageLatency);
		Provider stProvDa = new Provider(Entity.DiskArray, ProviderType.HOSTING, 1, 1);
		List<CommodityDTO> stCommBoughtFromDa = getCommList(Commodity.Extent, Commodity.StorageAccess, Commodity.StorageLatency);

		templateDTOSet.add( generateTemplateDTO(Entity.Storage, stCommSold,
				new CommBoughtPair(stProvDa, stCommBoughtFromDa)) );

		// PM DTO
		List<CommodityDTO> pmCommSold = getCommList(Commodity.CPU, Commodity.Mem, Commodity.IOThroughput, Commodity.ClusterCommodity, Commodity.NetThroughput, Commodity.DatastoreCommodity, Commodity.Q1VCPU, Commodity.DataCenterCommodity);

		Provider pmProvDc = new Provider(Entity.DataCenter, ProviderType.HOSTING, 1, 1);
		List<CommodityDTO> pmCommBoughtFromDc = getCommList(Commodity.Power, Commodity.Space, Commodity.Cooling);
		Provider pmProvSt = new Provider(Entity.Storage, ProviderType.LAYEREDOVER, Integer.MAX_VALUE, 0);
		List<CommodityDTO> pmCommBoughtFromSt = getCommList(Commodity.StorageAccess, Commodity.StorageLatency);

		templateDTOSet.add( generateTemplateDTO( Entity.PhysicalMachine, pmCommSold,
				new CommBoughtPair(pmProvDc, pmCommBoughtFromDc),
				new CommBoughtPair(pmProvSt, pmCommBoughtFromSt) ));

		// VM DTO
		List<CommodityDTO> vmCommSold = getCommList(Commodity.VCPU, Commodity.VMem, Commodity.ApplicationCommodity);
		List<CommodityDTO> vmCommBoughtFromPm = getCommList(Commodity.CPU, Commodity.Mem);
		List<CommodityDTO> vmCommBoughtFromSt = getCommList(Commodity.StorageAmount, Commodity.StorageProvisioned);
		Provider vmProvPm = new Provider(Entity.PhysicalMachine, ProviderType.HOSTING, 1, 1);
		Provider vmProvSt = new Provider(Entity.Storage, ProviderType.LAYEREDOVER, Integer.MAX_VALUE, 0);

		templateDTOSet.add( generateTemplateDTO( Entity.VirtualMachine, vmCommSold,
				new CommBoughtPair(vmProvPm, vmCommBoughtFromPm),
				new CommBoughtPair(vmProvSt, vmCommBoughtFromSt) ));

		// Application DTO
		List<CommodityDTO> appCommSold = getCommList();
		List<CommodityDTO> appCommBoughtFromVm = getCommList(Commodity.VCPU, Commodity.VMem, Commodity.ApplicationCommodity);
		...
		templateDTOSet.add( generateTemplateDTO( Entity.Application, appCommSold,
				new CommBoughtPair(appProvVm, appCommBoughtFromVm) ));
	...
	protected Map<String, AccountDefinitionEntry> getAccountDefinitionEntryMap() {
		...
		ImmutableMap<String, AccountDefinitionEntry> accountDefinitionEntryMap = ImmutableMap.of(
				AccountDefinitionEntry.TARGET_IDENTIFIER, new AccountDefinitionEntry(AccountDefinitionEntry.TARGET_IDENTIFIER, "File Name", "unique identifier of the target", AccountDefinitionEntryType.Mandatory, ".*")
				);
	...
	protected TargetValidationResponse validateTarget(Map<String, String> accountValues) {
		...
		if (inputStream != null) {
			...
			validationResponse.targetValidationExplanation = "File " + fileName + " does not exist.";
		}
